...
const SplitView: ParentComponent<{}> = (props) => {
  // Somehow using `HtmlElement` here causes a type error
  let leftSide: any;
  let rightSide: any;
  let resizer: any;

  const resolvedChildren = children(() => props.children) as Accessor<
    ResolvedJSXElement[]
  >;
  ...
  return (
    <Box h="$full" class="splitview-container">
      <div
        class="splitview-container-left"
        style={leftStyleGet()}
        ref={leftSide}
      >
        {resolvedChildren()[0]}
      ...
      <div
        class="splitview-resizer"
        style={resizerStyleGet()}
        ref={resizer}
        onMouseDown={mouseDownHandler}
      ></div>
      <div
        class="splitview-container-right"
        style={rightStyleGet()}
        ref={rightSide}
      >
        {resolvedChildren()[1]}
